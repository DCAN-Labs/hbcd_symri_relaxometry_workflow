...
def build_parser():
    ...
    parser.add_argument('--no_upload', help="If used, skip uploading results to S3 and keep outputs in custom_work_dir (or the work_dir under base_directory_for_proc). Implies --keep_work_dirs.", action='store_true')
...
def targz_to_sub_ses_labels(targz_name):
    ...
    if len(split_name) >= 3:
        if (len(split_name[0]) == 9) and (len(split_name[1]) == 6) and ((len(split_name[2]) == 3) or (len(split_name[2]) == 4)) and (split_name[2][0] in ['P', 'V']):
            if len(split_name) == 3:
...
def main():
    ...
    6. Upload the BIDS derivatives to a different S3 bucket (skipped if --no_upload is set).
    ...
    no_upload : bool, default False
        If used, skip uploading results to S3. Outputs are kept in custom_work_dir (or the
        work_dir under base_directory_for_proc). Implies --keep_work_dirs behavior for the
        session working directory so that results are not deleted after processing.
    ...
    # --no_upload implies keeping the work dirs so results aren't deleted
    if args.no_upload:
        args.keep_work_dirs = True
    ...
    for temp_session in sessions_to_evaluate.keys():
        ...
        if type(best_qalas_info) != type(None):
            ...
            # --no_upload: skip S3 transfer and keep outputs locally
            if args.no_upload:
                sys.stdout.write('   --no_upload set: skipping S3 upload. Results are in: {}\n'.format(output_info['bids_path']))
                sys.stdout.flush()
            else:
            ...
            #After BIDS conversion (+ optional upload), update the
